...
def generate_chat_title(first_user_message: str, has_image: bool = False) -> str:
    ...
    try:
        if has_image:
            prompt = f"다음 메시지를 16자 이내의 귀엽고 따뜻한 문구로 압축해줘. 사진도 함께 보냈어. 굵기 적용과 글자수 언급은 제외해줘.: {first_user_message}"
        else:
            prompt = f"다음 메시지를 16자 이내의 귀엽고 따뜻한 문구로 압축해줘. 굵기 적용과 글자수 언급은 제외해줘.: {first_user_message}"
