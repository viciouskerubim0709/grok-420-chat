...
def generate_chat_title(first_user_message: str, has_image: bool = False) -> str:
    ...
    try:
        if has_image:
            prompt = f"다음 메시지를 16자 이내의 귀엽고 따뜻한 문구로 표현해줘. 사진도 함께 보냈어. 굵기 적용과 글자수 언급은 제외해줘.: {first_user_message}"
        else:
            prompt = f"다음 메시지를 16자 이내의 귀엽고 따뜻한 문구로 표현해줘. 굵기 적용과 글자수 언급은 제외해줘.: {first_user_message}"
...
with st.sidebar:
    ...
    for chat_id, chat in list(st.session_state.chats.items()):
        ...
        with st.container(horizontal=True, horizontal_alignment="left", vertical_alignment="center", width="stretch"):
            label = "**[현재✨]** " + chat["title"] if is_current else chat["title"]
            if st.button(label, key=f"chat_{chat_id}", use_container_width=True, type="tertiary", width="stretch"):
